...
public class TemperatureScene {
    private Stage stage;

    public TemperatureScene(Stage stage) {
        this.stage = stage;
    }

    public void temperatureScene() {
        Text text3 = new Text("TEMPERATURE CONVERTER");
        ...
        calculateButton.setOnAction(v -> {
            TemperatureCalculate calculate = new TemperatureCalculate(cb1.getValue(), cb2.getValue(), Integer.parseInt(textField1.getText()));
            textField2.setText(Double.toString(calculate.convertTemperature()));
        });
